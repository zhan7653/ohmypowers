...
Every evidence.summary must be an exact, reviewable contiguous excerpt from the cited session, parsed finalized report, or reviewed memo after case and whitespace normalization. Never invent, infer, or paraphrase evidence summaries.
...
Classify an automatic instruction observed in only one repository as project_instruction for that repository. Only an automatic instruction supported by at least two projects may be global_instruction. Honor an explicit user-nominated scope, but never bypass confirmation.
For every candidate, set conflict, nestedScope, scopePath, and safetyReasons. Use conflict=true when available evidence indicates a possible conflict with an existing instruction. Use nestedScope=true and scopePath only when evidence indicates that a nested directory scope may be required. These signals make an instruction candidate pause-required and non-actionable until a human resolves them; they never silently change global/project classification. Do not invent target files, existing instruction contents, conflicts, nested paths, or scope facts that are absent from the supplied evidence.
