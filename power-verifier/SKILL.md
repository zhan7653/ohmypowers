...
7. Require at least one contract-conformance reviewer who did not participate in implementation before returning `PASS` or `PASS_WITH_NOTES`. A review plan may add code, test, security, compatibility, migration, data, or domain capabilities when the evidence justifies them. Record every reviewer’s identity or source, model and reasoning effort when exposed, model-selection rationale when supplied by the execution plan, independence, capabilities, scope, read-only boundary, evidence inspected, result, and snapshot.
