...
Small tasks stay in the main context. Judge inspection size by independent evidence questions, not by the expected diff: one or two total direct reads stay in the main context, while one bounded deep investigation or multiple independent read-only lanes can justify delegation. Before starting a second independent fact domain, apply a routing gate: if at least two stable, non-dependent domains each require more than one direct read, read [references/orchestration.md](references/orchestration.md) and dispatch all currently ready qualifying lanes in the same wave before continuing cross-domain evidence collection. Do not serialize those lanes in the main context and call the task small. This is a work-shape requirement, not an agent-count target; keep sequential dependencies, changing inputs, the immediate critical path, and synthesis in the main context, and never split work merely to increase the agent count. For clear implementation and test work, count preserving the main context for material judgment and avoiding unnecessary stronger-model work as secondary delegation benefits, but never delegate or split implementation or test work for cost alone; delivery quality and coordination safety come first. For other bounded independently useful work, non-overlapping writes, or a genuinely independent review, read the same reference and route by task shape across `power_worker`, `power_scout`, `power_explorer`, `power_planner`, and `power_reviewer`. Each profile owns its own model and effort configuration; do not restate or override them here.
