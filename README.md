...
- If audit persistence fails, the target is restored to its exact prior bytes or absence and `audit_persistence_failed` is reported as rolled back, never successful. `rollback_failed` pauses for inspection of the target and error evidence; it is never retried or reported as successful automatically.
