...
From the first material item, `$power-gan` maintains one goal-scoped Decision Ledger under `${CODEX_HOME:-$HOME/.codex}/power-gan/records/<repository-key>/<delivery-id>/decision-snapshot.md`. During alignment and delivery, version 3 Ledgers retain confirmed, pending, delegated, rejected, and superseded material boundaries together with sourced basis, presented recommendation, and resolution evidence. Follow-up work on the same outcome reuses the delivery identity across threads; an independent outcome receives another delivery ID. Every user answer is reconciled before the next question or task action, and resumed contexts re-read the Ledger instead of rebuilding decisions from conversational memory. Version 2 and versionless legacy Ledgers are never migrated or compacted automatically.
...
Before the first source write, the Ledger receives the completion basis, hard constraints, current smallest approach, validation direction, stop conditions, and final carrier. For version 3, the bundled validator derives a launch projection containing only those launch fields and active confirmed/delegated decision statements; recommendations, process evidence, rejected/superseded history, metadata, and Working defaults stay outside the projection and its SHA-256. The agent forwards the marker-delimited projection verbatim. A direct request to implement establishes delivery intent only; source writing starts only after the user explicitly confirms that complete projection. A change to active content invalidates the confirmation, while excluded history changes do not.
...
After verified handoff, an ordinary version 3 delivery replaces its full Ledger with a validated compact index containing repository/delivery identity, the next unused decision ID, verified decision source, final carrier, final decision digest, and handoff evidence. Full structured history remains only when explicitly requested or when the delivery materially affects security, privacy, permissions, production persistent state, data migration, cross-version compatibility, concurrency correctness, irreversible behavior, or material cost. Handoff failures keep the full Ledger. The workflow does not maintain a second Journal or repository-local decision database.
