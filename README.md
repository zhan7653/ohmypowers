...
- Compares current evidence with up to 10 readable finalized reports and renders personal reflection plus Skill, automation, global-instruction, and project-instruction candidates with evidence, scope, and confirmation status. Evidence summaries are exact source-bound excerpts from cited sessions, finalized reports, or reviewed memos—not paraphrases—so they require privacy review before sharing.
- Renders explicit candidate conflict, nested-scope, scope-path, and safety-reason signals. Those signals pause for human scope/rule resolution and flow into typed `conflict` or `nested_scope` refusal; they are not a claim that every semantic conflict can be understood automatically.
...
- Target and memory updates use no-overwrite compare-and-commit. If concurrent memory change prevents audit persistence, the concurrent memory is preserved, the target is restored to its exact prior bytes or absence, and `audit_persistence_failed` is reported as rolled back, never successful; fresh review, plan, and confirmations are required. `rollback_failed` pauses for inspection of the target and error evidence and is never retried or reported as successful automatically.
- Generic hard-link failure uses an exclusive, verified no-replace copy fallback. A verified restoration still reports the failed operation and never claims success. `recovery_failed` or `memory_recovery_failed` means the live file may be unavailable while exact prior bytes remain in the named quarantine/recovery evidence; inspect the live path, quarantine, and errors, then pause without automatic retry or rollback/success claims.
