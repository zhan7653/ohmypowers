...
Small tasks stay in the main context. Before `$power-gan` starts a second independent fact domain, two stable, non-dependent domains that each need more than one direct read trigger same-wave delegation; one or two total reads and sequential dependencies stay in the main context. For delegation, `$power-gan` routes clear implementation and tests to the managed Terra/high `power_worker`, bounded evidence collection and summarization to the behaviorally read-only Terra/medium `power_scout`, multi-hypothesis exploration to the behaviorally read-only Sol/medium `power_explorer`, and genuinely ambiguous planning or cross-agent synthesis to the behaviorally read-only Sol/xhigh `power_planner`. Completed implementation review and required `$power-check` use the uniquely named, behaviorally read-only Sol/high `power_reviewer`. Each profile owns its model, reasoning effort, and behavioral constraints; the workflow does not rely on the host sandbox being downgraded for a child.

Routing and task packets remain reversible implementation details: they are not persisted as an Agent Dispatch Plan or put through a user confirmation loop. The main context keeps material decisions, final arbitration, and user communication. For clear implementation and test work, preserving that context for material judgment and avoiding unnecessary stronger-model work are secondary `power_worker` delegation benefits, but cost alone never justifies worker delegation or splitting implementation or test work; delivery quality and coordination safety remain primary. If a managed profile is unavailable or its configuration cannot be honored, the workflow uses generic delegation or the main context only when that is still adequate; it does not claim an exact model or independent context that was not provided.
